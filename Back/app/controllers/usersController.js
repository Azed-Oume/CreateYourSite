...
async function generateResetLink(user) {
    console.log(user.email, " en ligne 29 XXXXXXXXXXXX");
    const email = user.email;
    let utilisateur = await User.findOne({ where: { email } });
    console.log(utilisateur, " en ligne 32 XXXXXXXXXXXXX");

    const token = jwt.sign({ userId: utilisateur.utilisateur_id }, secretKey, { expiresIn: '1h' });
    console.log(utilisateur.utilisateur_id, "en ligne 37 XXXXXXXXX");

    const resetLink = `http://localhost:1234/api/reset-password/${token}`;  // Correction de l'URL
    console.log(resetLink, " en ligne 20 XXXXXXXXXXXXXXXX");
    return resetLink;
}
...
async function sendResetEmail(to, link) {
    ...
    });console.log(emailUser, emailPass, " en ligne 87 XXXXXXXXXXXXXX");
...
resetPassword: async (req, res) => {
    ...
    getUser: async function (req, res) {
        ...
        try {
            ...
            console.log(userId, "en ligne 67");
    ...
    getSociete: async function (req, res) {
        ...
        try {
            ...
            console.log(userId, "en ligne 90 XXXXXXXXXXXXX");
            // Rechercher l'utilisateur dans la base de données en utilisant son ID
            const user = await User.findByPk(userId, {transaction} );
            console.log(userId, " en ligne 93 XXXXXXXXXXXX")
            if (!user) {
                await transaction.rollback();
                return res.status(404).json({ message: 'Utilisateur non trouvé' });
            }
    ...
    updateUser: async function (req, res) {
        ...
        try {
            ...
            console.log(userId, " en ligne 114 XXXXXXXXXXXXXX");

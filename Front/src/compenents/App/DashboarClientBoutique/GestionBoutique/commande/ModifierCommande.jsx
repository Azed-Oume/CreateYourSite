...
const ModifierCommande = () => {
    ...
    useEffect(() => {
      if (data && data.commande && data.commande.Produits && data.quantites) {
        const nouveauPanier = data.commande.Produits.map(produit => {
  ...
  useEffect(() => {
    if (client) {
        ...
        const generateNumeroCommande = () => {
            ...
            const numeroCommande = `${annee}${mois}${jour}${minutes}${seconds}/${nomClient}${prenomClient}/${randomDigits}`;
  ...
  const handleDownloadPDF = async () => {
    ...
    pdf.text(`Commande numéro: ${data.commande.numero_commande}`, 70 ,90); // Affiche le numéro de devis à la position spécifiée
    ...
    pdf.text(`Commande à régler !`, 140, pdf.autoTable.previous.finalY + 20); // Affichage de la validité du devis
    ...
    const mentionsLegales = [
        ...
        `Conditions de paiement : ${conditionsPaiement} : Validité du devis : ${validateCommande} jours`,
        ...
        "Engagement de paiement : Cette commande est ferme et vous engage à régler la somme due.",
        ...
        "Acceptation : L'acceptation de cette commande par voie électronique vaut acceptation expresse du client."
    ];
    ...
    pdf.save(`Commande${data.commande.numero_commande}.pdf`);
    
    await envoyerCommande(data.commande.numero_commande, data.commande.commande_id, panier);
};




  // Fonction pour envoyer, supprimer puis regénérer la commande en base de données
const envoyerCommande = async (numeroCommande, commandeId, panierDeCommande) => {
  // Vérifiez que panierDeCommande est défini et est un tableau
  if (!Array.isArray(panierDeCommande)) {
    console.error('Le panier de commande n\'est pas défini ou n\'est pas un tableau');
    return;
  }
  
  // Vérifiez que le panier n'est pas vide
  if (panierDeCommande.length === 0) {
    throw new Error('Le panier est vide');
  }

  try {
    const token = localStorage.getItem('token');
    const response = await fetch(`${import.meta.env.VITE_URL_API}/api/update/commande/${commandeId}`, {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({
        ...
        validateCommande,
        detailProjet,
        panier: panierDeCommande.map(produit => ({
          nom: produit.nom,
          tarif: produit.tarif,
...
const conditionsPaiement = "A la confirmation de la commande !"
    ...
    return (
        ...
        <Form className='graylogo col-md-10 mx-auto p-4 gap-2'>
            ...
            <table className="table mt-4">
                ...
                <tbody>
                    {Array.isArray(data.commande.Produits) && data.commande.Produits.map((produit, index) => {
            ...
            <nav className='d-flex justify-content-between'>
                ...
                <Button
                    ...
                    onClick={() => {
                        handleDownloadPDF();
                        // envoyerDevis(data.devis.numero_devis, data.devis.devis_id, panier);
                    }}>
